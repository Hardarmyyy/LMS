...
import UsevalidateRegisterStudent from './UsevalidateRegisterStudent'
...
const UseRegisterStudent = () => {
    ...
    const handleChange = (e) => {
        ...
        if (name === 'status') {
            setError((error) => {return {...error, status: value ? '': 'Kindly select status' }})
        }
        if (name === 'session') {
            setError((error) => {return {...error, session: value ? '': 'Kindly choose session' }})
        }
        if (name === 'class') {
            setError((error) => {return {...error, class: value ? '': 'Kindly choose class' }})
        }
        if (name === 'origin') {
    ...
    //UsevalidateRegisterStudent hook to catch errors on the form object;
    const {errors} = UsevalidateRegisterStudent(user)
    ...
    // define a function to dispatch the register student request
    ...
    return {user, error, handleChange, handleRegisterStudent}
}
